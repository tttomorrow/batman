    ...
    <template v-if="$route.meta.isTab">
      ...
      <el-tabs v-model="$store.state.contentTabsActiveName" @tab-click="tabSelectedHandle" @tab-remove="tabRemoveHandle">
        <el-tab-pane
          ...
          :name="item.name"
          :label="item.title"
          :closable="item.name !== 'sys-instance'"
...
export default {
  ...
  methods: {
    ...
    tabRemoveHandle (tabName) {
      if (tabName === 'sys-instance') {
        return false
      }
      this.$store.state.contentTabs = this.$store.state.contentTabs.filter(item => item.name !== tabName)
      if (this.$store.state.contentTabs.length <= 0) {
        this.$store.state.sidebarMenuActiveName = this.$store.state.contentTabsActiveName = 'sys-instance'
        return false
      }
      // 当前选中tab被删除
      if (tabName === this.$store.state.contentTabsActiveName) {
        let tab = this.$store.state.contentTabs[this.$store.state.contentTabs.length - 1]
        this.$store.state.sidebarMenuActiveName = tab.name
        this.$router.push({
          name: tab.name,
          params: { ...tab.params },
          query: { ...tab.query }
    ...
    tabsCloseOtherHandle () {
      this.$store.state.contentTabs = this.$store.state.contentTabs.filter(item => {
        return item.name === 'sys-instance' || item.name === this.$store.state.contentTabsActiveName
      })
    },
    // tabs, 关闭全部
    tabsCloseAllHandle () {
      this.$store.state.contentTabs = this.$store.state.contentTabs.filter(item => item.name === 'sys-instance')
      this.$router.push({ name: 'sys-instance' })
    }
  }
}
</script>
