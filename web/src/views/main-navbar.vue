<template>
  <nav class="aui-navbar" :class="`aui-navbar--${$store.state.navbarLayoutType}`">
    <div class="aui-navbar__header">
      <h1 class="aui-navbar__brand" @click="$router.push({ name: 'sys-instance' })">
        <a class="aui-navbar__brand-lg" href="javascript:;">{{ $t('brand.batman') }}</a>
  ...
  methods: {
    ...
    logoutHandle () {
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('logout') }), this.$t('prompt.title'), {
        ...
        this.$http.post('/sys/logout').then(({ data: res }) => {
          if (res.code !== 0) {
